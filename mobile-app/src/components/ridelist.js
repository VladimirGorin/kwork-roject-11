            ...
            <TouchableOpacity style={[styles.iconClickStyle,{flexDirection:isRTL?'row-reverse':'row'}]} onPress={() => onPressButton(item, index)}>
                ...
                <View style={[styles.flexViewStyle, isRTL?{flexDirection:'row-reverse',marginRight: 5}:{flexDirection:'row',marginLeft: 5}]}>
                    ...
                    <View style={styles.textView2}>
                        ...
                        <Text style={[styles.fareStyle, styles.dateStyle,{textAlign:isRTL? "right":"left"}]}>{item.status == 'PAID' || item.status == 'COMPLETE'? item.customer_paid ? settings.symbol + parseFloat(item.customer_paid).toFixed(2) : settings.symbol + parseFloat(item.estimate).toFixed(2) : null}</Text>
                        {
