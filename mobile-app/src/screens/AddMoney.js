...
export default function AddMoneyScreen(props) {
  ...
  const newData = ({ item, index }) => {
    return (
      <TouchableOpacity style={[styles.boxView, { backgroundColor: item.selected ? colors.BUTTON_BACKGROUND : colors.BACKGROUND_PRIMARY }]} onPress={() => { quckAdd(index); }}><Text style={styles.quckMoneyText, { color: item.selected ? colors.WHITE : colors.BLACK }} >{settings.symbol}{item.amount}</Text></TouchableOpacity>
    )
  }
  ...
  return (
    <View style={styles.mainView}>
      ...
      <View style={styles.bodyContainer}>
        ...
        <Text>- </Text>
        <Text style={styles.ballance}>{settings.symbol}{state.userdata ? parseFloat(state.userdata.walletBalance).toFixed(2) : ''}</Text>
      </View>
