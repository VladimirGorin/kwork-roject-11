                    ...
                    <View style={styles.containsView}>
                        <View style={[styles.statusStyle, {flexDirection:isRTL?'row-reverse':'row'}]}>
                            ...
                            {item.type  && item.type == 'Credit'?
                                <Text style={styles.historyamounttextStyle, {textAlign:isRTL? "right":"left"}}>{t('credited') + ' ' + settings.symbol + parseFloat(item.amount).toFixed(2)}</Text>
                            :null}
                            {item.type && item.type == 'Debit'?
                                <Text style={styles.historyamounttextStyle, {textAlign:isRTL? "right":"left"}}>{t('debited') + ' ' + settings.symbol + parseFloat(item.amount).toFixed(2)}</Text>
                            :null}
                            {item.type && item.type == 'Withdraw'?
                                <Text style={styles.historyamounttextStyle, {textAlign:isRTL? "right":"left"}}>{t('withdrawn') + ' ' + settings.symbol + parseFloat(item.amount).toFixed(2)}</Text>
                            :null}   
