...
export default function WalletDetails(props) {
  ...
  const rCom = auth.info && auth.info.profile && auth.info.profile.usertype =='driver'?<TouchableOpacity onPress={doWithdraw}><Text style={{color:colors.WHITE}}>{t('withdraw')}</Text></TouchableOpacity>:null;
          ...
          <View >
            <View style={{ flexDirection:isRTL?'row-reverse':'row', justifyContent: "space-around", marginTop: 8 }}>
              <View style={{ height: walletBar - 50, width: '48%', backgroundColor: colors.BORDER_BACKGROUND, borderRadius: 8, justifyContent: 'center', flexDirection: 'column' }}>
                ...
                <Text style={{ textAlign: 'center', fontSize: 25, fontWeight: '500', color: colors.BALANCE_GREEN }}>{settings.symbol}{auth.info && auth.info.profile ? parseFloat(auth.info.profile.walletBalance).toFixed(2) : ''}</Text>
              </View>
