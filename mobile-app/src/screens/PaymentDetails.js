...
export default function PaymentDetails(props) {
  ...
  const promoHeaderCom = { icon: 'ios-close', type: 'ionicon', color: colors.WHITE, size: 45, component: TouchableWithoutFeedback, onPress: () => { setPromodalVisible(false) } };
        ...
        <View style={{ flex: 1, flexDirection: 'column' }}>
          ...
          {userdata ?
            <View style={{ flex: 1, flexDirection: isRTL?'row-reverse':'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              ...
              <Text style={{ color: colors.BLACK, textAlign: isRTL?'right':'left', lineHeight: 45, fontSize: 16 }}>{settings.symbol} {parseFloat(payDetails.amount).toFixed(2)}</Text>
            </View>
            : null
          }
          {userdata ?
            <View style={{ flex: 1, flexDirection:isRTL?'row-reverse': 'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              ...
              <Text style={{ color: colors.DULL_RED, textAlign:isRTL?'right': 'left', lineHeight: 45, fontSize: 16 }}>{isRTL? null:'-'} {settings.symbol} {payDetails ? payDetails.discount ? parseFloat(payDetails.discount).toFixed(2) : '0.00' : '0.00'} {isRTL? '-':null}</Text>
            </View>
            : null}
          ...
          {useWalletCash || (booking.status != 'PAYMENT_PENDING' && booking.usedWalletMoney)?
            <View style={{ flex: 1, flexDirection: isRTL?'row-reverse':'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              ...
              <Text style={{ color: colors.DULL_RED, textAlign: isRTL?'right':'left', lineHeight: 45, fontSize: 16 }}>{isRTL? null:'-'} {settings.symbol} {payDetails ? payDetails.usedWalletMoney ? parseFloat(payDetails.usedWalletMoney).toFixed(2) : '0.00' : '0.00'} {isRTL? '-':null}</Text>
            </View> : null}
          ...
          {userdata ?
            <View style={{ flex: 1, flexDirection: isRTL?'row-reverse':'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              ...
              <Text style={{ color: colors.START_TRIP, textAlign:isRTL?'right':'left', lineHeight: 45, fontSize: 24, fontWeight: 'bold' }}>{settings.symbol} {payDetails.payableAmount ? parseFloat(payDetails.payableAmount).toFixed(2) : 0.00}</Text>
            </View>
            : null}
        </View>
