      ...
      Object.keys(mainArr).map(j => {
        if ((mainArr[j].status === 'PAID' || mainArr[j].status === 'COMPLETE') && mainArr[j].discount_amount !== undefined && mainArr[j].driver_share !== undefined && mainArr[j].customer_paid !== undefined && mainArr[j].convenience_fees !== undefined && mainArr[j].trip_cost !== undefined) {
          ...
          if (renderobj[uniqueKey]) {
            ...
            renderobj[uniqueKey]['discountAmount'] = parseFloat(mainArr[j].discount_amount).toFixed(2);
            renderobj[uniqueKey]['driverShare'] = parseFloat(mainArr[j].driver_share).toFixed(2);
            renderobj[uniqueKey]['customerPaid'] = parseFloat(mainArr[j].customer_paid).toFixed(2);
            renderobj[uniqueKey]['convenienceFee'] = parseFloat(mainArr[j].convenience_fees).toFixed(2);
            renderobj[uniqueKey]['uniqueKey'] = uniqueKey;
            renderobj[uniqueKey]['tripCost'] = parseFloat(mainArr[j].trip_cost).toFixed(2);
